...
检查、测试和操作结果先显示完整的中文说明，再显示独立的英文区块，不会逐句交替。错误的技术详情放在最后。以下是各状态的含义：

| 输出 | 含义 |
| --- | --- |
| 待修复 / PATCHABLE | 本地规则仍包含多余字段，需要应用补丁。 |
| 规则已修复 / RULE FIXED | 本地规则已无嵌套 `reasoning`，无需重复应用；不代表运行中的 ZCode 已重新加载。 |
| 通过 / PASS | 这一组离线测试通过，不代表补丁已经安装。 |
| 未通过 / FAIL | 测试没有通过，查看下方原因；例如集成测试要求安装文件尚未打补丁。 |
| 失败 / ERROR | 文件读取或校验失败，检查路径、权限及规则版本。 |

`verify` 只检查本地文件，不会向模型发送请求。正常读到“待修复”也会返回退出码 0；读取或校验失败才返回非零值。`test` 中任何一组失败都会返回非零值。详细技术状态 `patchable` / `no-nested-reasoning` 仍会显示，方便反馈问题。
...
如果本机装了 `make`，可以使用对应命令。Makefile 会先检查系统里是否有 `bun`：有 Bun 时使用 `bun run`，没有时自动回退到 `npm run`。
